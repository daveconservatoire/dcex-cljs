...
(deftest parser-read-courses
  (async done
    (go
      (is (= (<! (p/parse {::l/db ts/connection} [:app/courses]))
             {:app/courses [{:db/id 4 :db/table :course} {:db/id 7 :db/table :course}]}))

      (is (= (<! (p/parse {::l/db ts/connection} [{:app/courses [:db/id :course/title]}]))
      ...
      (is (= (<! (p/parse {::l/db ts/connection} [{:app/courses [:db/id :course/home-type]}]))
      ...
      (is (= (<! (p/parse {::l/db ts/connection} [{:app/courses {:course.type/multi-topic [:db/id :course/home-type]
...
(deftest test-parse-read-lesson-by-slug
  (async done
    (go
      (is (= (->> (p/parse {::l/db ts/connection} [{[:lesson/by-slug "percussion"] [:db/id :lesson/title]}]) <!)
             {[:lesson/by-slug "percussion"] {:db/id 9 :db/table :lesson :lesson/title "Percussion"}}))
      (is (= (->> (p/parse {::l/db ts/connection} [{[:lesson/by-slug "percussion"] [:db/id :lesson/type]}]) <!)
             {[:lesson/by-slug "percussion"] {:db/id 9 :db/table :lesson :lesson/type :lesson.type/video}}))
      (is (= (->> (p/parse {::l/db ts/connection} [{[:lesson/by-slug "invalid"] [:db/id :lesson/title]}]) <!)
...
(deftest parser-read-route-data
  (async done
    (go
      (is (= (<! (p/parse {::l/db ts/connection} [{:route/data [:app/courses]}]))
             {:route/data {:app/courses [{:db/id 4 :db/table :course} {:db/id 7 :db/table :course}]}}))
      (is (= (<! (p/parse {::l/db ts/connection} [{:ph/anything [:app/courses]}]))
...
(deftest test-read-me
  (async done
    (go
      (is (= (->> (p/parse {::l/db ts/connection :current-user-id 720} [{:app/me [:db/id]}]) <!
                  :app/me)
             {:db/id 720 :db/table :user}))
      (is (= (->> (p/parse {::l/db ts/connection} [{:app/me [:db/id]}]) <!
                  :app/me)
             nil))
      (done))))
