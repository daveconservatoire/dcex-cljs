...
[{[:video/by-id "mS264h8KGwk"]
  [:video/title :video/published-at :video/description :video/view-count
   :video/like-count :video/dislike-count
   {:video/channel [:channel/title :channel/logo :channel/subscribers-count]}
   {:video/comments [:comment/text :comment/created-at :comment/like-count
                     :comment/dislike-count
                     {:comment/author [:user/name :user/thumbnail]}]}
   {:video/related [:video/title :video/thumbnail :video/view-count
                    :video/published-at
                    {:video/channel [:channel/title]}]}]}]

(om/defui ^:once CommentAuthor
  static om/IQuery
  (query [_] [:user/name :user/thumbnail])

  Object
  (render [this]))

(om/defui ^:once Comment
  static om/IQuery
  (query [_] [:comment/text :comment/created-at :comment/like-count :comment/dislike-count
              {:comment/author (om/get-query CommentAuthor)}])

  Object
  (render [this]))

(om/defui ^:once ChannelDescription
  static om/IQuery
  (query [_] [:channel/title :channel/logo :channel/subscribers-count])

  Object
  (render [this]))

(om/defui ^:once ChannelTitleQuery
  static om/IQuery
  (query [_] [:channel/title])

  Object
  (render [this]))

(om/defui ^:once RelatedVideo
  static om/IQuery
  (query [_] [:video/title :video/thumbnail :video/view-count
              :video/published-at
              {:video/channel (om/get-query ChannelTitleQuery)}])
...
(om/defui ^:once Video
  static om/IQuery
  (query [_] [:video/title :video/published-at :video/description :video/view-count
              :video/like-count :video/dislike-count
              {:video/channel (om/get-query ChannelDescription)}
              {:video/comments (om/get-query Comment)}
              {:video/related (om/get-query RelatedVideo)}])
